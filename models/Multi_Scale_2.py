...
class m_unet(nn.Module):
    ...
    def __init__(self, n_class=1):
        ...
        self.conv6_u = double_conv(240,16)
        self.conv7_u = double_conv(96,16)
    ...
    def forward(self,x1,x2,x3):
        ...
        u5 = self.conv5_u(u4)
        u5 = self.upsample(u5)
        u5 = torch.cat([u5, conv1_3,conv2_2,conv3_1], dim=1)
        
        # ### Block 6 ###
        u6 = self.conv6_u(u5)
        u6 = self.upsample(u6)
        u6 = torch.cat([u6,conv1_2,conv2_1], dim=1)
        
        # ### Block 7 ###
        u7 = self.conv7_u(u6)
        u7 = self.upsample(u7)
        u7 = torch.cat([u7,conv1_1], dim=1)
